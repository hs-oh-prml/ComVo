...
from models.modules import safe_log
...
class cFeatureMatchingLoss(nn.Module):
    ...
    def forward(
        ...
        for dr, dg in zip(fmap_r, fmap_g):
            for rl, gl in zip(dr, dg):
                l1_re = torch.mean(torch.abs(rl.real - gl.real))
                l1_im = torch.mean(torch.abs(rl.imag - gl.imag))
                loss += 0.5 * (l1_re + l1_im)
...
class cGeneratorLoss(nn.Module):
    ...
    def forward(
        ...
        for dg in disc_outputs:
            ...
            l = 0.5 * (
                torch.relu(1.0 - dg.real).mean() + torch.relu(1.0 - dg.imag).mean()
            )
            gen_losses.append(l)
            loss += l
...
class cDiscriminatorLoss(nn.Module):
    def forward(
        ...
        g_losses = []
        for dr, dg in zip(disc_real_outputs, disc_generated_outputs):
            r_loss = torch.relu(1.0 - dr.real).mean() + torch.relu(1.0 - dr.imag).mean()
            
            g_loss = torch.relu(1.0 + dg.real).mean() + torch.relu(1.0 + dg.imag).mean()
            l = 0.5 * (r_loss + g_loss)
            loss += l
            r_losses.append(r_loss)
            g_losses.append(g_loss)
